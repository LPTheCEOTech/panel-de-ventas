...
| Fecha | Nota |
|---|---|
| 2026-09-10 | **Código completo.** Migración 005 (tabla `usuarios` con FK a auth.users cascade + FK a personas restrict + índice único parcial por persona_id + rol enum admin/miembro). Tipos `RolUsuario`, `Usuario`, `Sesion`. Capa Supabase + Demo agregan `buscarUsuario/crearUsuario/borrarUsuario` y `personaId?` en `leerReportes*`. Helper `sesionActual()` en `sesion-usuario.ts` (usa `cookies()` de next/headers para armar el cliente anon y `getUser()` para verificar; luego busca en `usuarios` con service_role). Layout `(app)` redirige a `/pendiente` si hay auth user pero no fila en `usuarios`. Pantalla `/pendiente` fuera de `(app)` con botón Salir. Nav filtra Gasto/Equipo/Ajustes cuando rol=miembro. Topbar muestra correo/persona + rol del logueado (fallback a config sin sesión). Panel filtra reportes por personaId de miembro y oculta Costos + Ranking + Gasto en el embudo (paso 0). Ajustes/Gasto/Equipo redirigen a `/panel` si no admin. Formularios setter/closer aceptan `bloqueadoA` → reemplazan `<select>` por `.confirma-persona`. Endpoints setter/closer overridean el `personaId` del body por el de la sesión cuando rol=miembro (nunca confiar en el body para authz). Guardia server-side `soloAdmin()` en `/api/gasto`, `/api/ajustes`, `/api/equipo`, `/api/logo`. Endpoint `POST /api/equipo/invitar` (`sb.auth.admin.inviteUserByEmail` + reuso/creación de persona + crearUsuario miembro). UI de invitar en `lista.tsx` reusando nombre+rol de "Agregar". Instalador agrega `005` a `MIGRACIONES` y crea la fila `usuarios(auth_user_id, null, 'admin')` para el usuario que crea (o los preexistentes en re-instalación). Mockup con `.confirma-persona`. Gate local: typecheck, 32/32 tests, lint, sin-cliente (61 archivos), build (11/11 páginas). Decisiones: **(1)** Se KEPT `usuario_nombre`/`usuario_rol` en `configuracion` en vez de dropearlos (§10 opción a) — riesgo menor a mitad de vuelo, el Topbar los ignora cuando hay sesión activa. **(2)** Ranking oculto al miembro (§4 opción a, recomendada por el PRP). **(3)** El proxy sigue sin chequear `usuarios` — solo mira si el auth user existe (mantiene edge/middleware fino); el layout hace el chequeo real. |
| 2026-09-10 | **Falta contra base real:** migración 005, correr instalador para insertar admin en `usuarios`, invitar un correo de prueba (requiere SMTP configurado en Supabase Auth), verificar logueo como miembro con panel filtrado, y probar contraprueba con curl (`personaId` ajeno desde sesión miembro → tiene que guardarse con el propio). |
