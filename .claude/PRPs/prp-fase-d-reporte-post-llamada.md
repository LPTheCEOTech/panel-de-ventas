...
| Fecha | Nota |
|---|---|
| 2026-09-10 | **Código completo.** Migración 006 crea `llamadas` (persona_id FK restrict, activa=true por default para baja lógica, revenue/cash con default 0, índices parciales `where activa`). Nuevo tipo `Llamada`. Kernel gana **`agregarLlamadas(llamadas)`** — el puente que convierte 96 llamadas en 21 filas agregadas para reusar `totales/metricas/rankings/barras` sin tocar sus firmas. Semilla nueva: **`LLAMADAS_DEMO` con 96 filas** deterministas (id `demo-<closer>-<fecha>-<n>` para upsert idempotente), calibradas para sumar EXACTO al ORO. Distribución: primeras C llamadas del día son `cerro=true` con revenue $3.000 cada una y cash repartido entero (el resto va a la primera cierre); días con C=0 y cash>0 (Carlos días 3, 6 y 7 — cobros de ventas anteriores) toda la plata cae en la primera asistida. 5 tests nuevos en `metricas.test.ts`. Capa Supabase + Demo con `leerLlamadas/crearLlamada/actualizarLlamada/bajaLogicaLlamada`. Endpoint `POST/PATCH/DELETE /api/llamadas` con override server-side de personaId por sesión (misma barrera que Fase C) y verificación de propiedad en PATCH/DELETE. Pantalla `/llamada` con `PanelLlamada`: form chico (3 switches + 2 CampoDinero + nota) + tarjeta lateral con contador vivo (llamadas/asistieron/cierres/tasa/cash/contratado del día) + lista abajo con badges (asistió/reagendada/cerró) + baja lógica con confirmación inline. Ruta `/reporte-closer` reemplazada por `redirect('/llamada')` — decisión Opción (a) del §5.2: `reportes_closer` queda como legacy en la BD hasta la 007. Panel `page.tsx` lee `llamadas` → `agregarLlamadas` → mismos rankings/barras. Nav: «Reporte Closer» → **«Post Llamada»** apuntando a `/llamada`. Verificador `verificar.mjs` cuenta `filasSetter` y `filasLlamadas` por separado (21 + 96) y lee `llamadas` con `activa=true`. Semilla script planta las 96 llamadas con `onConflict:'id'` (idempotente); limpia `llamadas` best-effort. Instalador agrega `006` a `MIGRACIONES`. Mockup: nav actualizada, pantalla nueva `?s=llamada` con lista de 4 llamadas de ejemplo + contador vivo, CSS `.switches-llamada` (píldoras que se colorean con `:has(input:checked)`) y `.llamada-row` en grid. Gate local: typecheck, **37/37 tests** (5 nuevos de Fase D + los 32 previos), lint, sin-cliente (65 archivos), build (12/12 páginas). **Decisión:** en vez de cambiar la firma de `totales/metricas/rankings/barras` (opción del PRP), se agregó el puente `agregarLlamadas()`. Menor blast radius, tests intactos, cambio localizado. |
| 2026-09-10 | **Falta contra base real:** migración 006, re-correr `npm run semilla -- limpiar && npm run semilla -- cargar` para plantar las 96 llamadas, correr `npm run verificar` (los 24 anteriores + 4 gasto + filasSetter + filasLlamadas), contraprueba (bajar una llamada `activa=false` y ver `verificar` en rojo), foto en 4 modos de `/llamada` (vacía / con lista / durante borrado). |
